...
//   }
  ...
  while (alpha >= 180){
    ...
    v_T_R = vB[0] * (w[0]/normr) + vB[1] * (w[1]/normr);
    if((vB[0] * (r[0]/normr) + vB[1] * (r[1]/normr)) < 0){ v_T_R = 0; }
  ...
  vB[0] = polarPlot[(mwSize)floor(thetaBoat_R_max - windDir)] * cos((thetaBoat_R_max)*(PI/180.0));
  vB[1] = polarPlot[(mwSize)floor(thetaBoat_R_max - windDir)] * sin((thetaBoat_R_max)*(PI/180.0));
  v_T_R_max = vB[0] * (r[0]/normr) + vB[1] * (r[1]/normr);
  ...
  while (alpha <= 180){
    ...
    //    
    //calculate velocity towards target
    v_T_L = vB[0] * (w[0]/normr) + vB[1] * (w[1]/normr);
    if((vB[0] * (r[0]/normr) + vB[1] * (r[1]/normr)) < 0){ v_T_L = 0; }
  ...
  vB[0] = polarPlot[(mwSize)floor(thetaBoat_L_max - windDir)] * cos((thetaBoat_L_max)*(PI/180.0));
  vB[1] = polarPlot[(mwSize)floor(thetaBoat_L_max - windDir)] * sin((thetaBoat_L_max)*(PI/180.0));
  v_T_L_max = vB[0] * (r[0]/normr) + vB[1] * (r[1]/normr);
  
  float atann = 90;
  if(r[0] > 0.1 || r[0] < -0.1){ atann = floor(atan(r[1]/r[0])*(180/PI)); }
  else if(r[1] < 0){ atann = 270;}
  
  if(r[1] == 0 && r[0] < 0){ atann = 180;}
  else if(r[0] < 0 && r[1] < 0){ atann = atann + 180;}
  mwSize deg = (mwSize) (atann);
  if(deg < 0) { deg = deg + 360; }
  mwSize wdeg = deg - ((mwSize) floor(windDir));
  if(wdeg < 0) { wdeg = wdeg + 360; }
  vB[0] = polarPlot[(wdeg)] * cos((windDir + wdeg)*(PI/180.0));
  vB[1] = polarPlot[(wdeg)] * sin((windDir + wdeg)*(PI/180.0));
  
  if (polarPlot[(wdeg)] > 0.2 && (vB[0] * (r[0]/normr) + vB[1] * (r[1]/normr)) > 0){
      if(wdeg > 180){ 
          thetaBoat_R_max = deg;
          vB[0] = polarPlot[(wdeg)] * cos((windDir + wdeg)*(PI/180.0));
          vB[1] = polarPlot[(wdeg)] * sin((windDir + wdeg)*(PI/180.0));
          v_T_R_max = 3 * vB[0] * (r[0]/normr) + 3 * vB[1] * (r[1]/normr); 
          v_T_R_max = 100.0;
      }
      else{
          thetaBoat_L_max = deg;
          vB[0] = polarPlot[(wdeg)] * cos((windDir + wdeg)*(PI/180.0));
          vB[1] = polarPlot[(wdeg)] * sin((windDir + wdeg)*(PI/180.0));
          v_T_L_max = 3 * vB[0] * (r[0]/normr) + 3 * vB[1] * (r[1]/normr);
          v_T_L_max = 100.0;
      }
  }
  //  
...
//   }
  ...
  angles[1] = sailAngle;
  //angles[1]= thetaBoat_L_max;
  angles[2] = prevErr;
  //angles[2] = thetaBoat_R_max;
  //angles[3] = opt;
  //angles[3] = (mwSize) floor(atann - windDir + 360);
  //angles[3] = (vB[0] * (r[0]/normr) + vB[1] * (r[1]/normr));
  angles[3] = wpNum;
}
