function [t,stateArray0]=odeEuler2(fhandle,stateArray0,ptemp)
...
%LOOP BEGINS--------ALEC EDITS: Replaced manual input with nav code

%Would it be easier to implement an algorithm
%that adjusted the sail by moving it up and
%down instead of to specific locations?
currpos=[stateArray0(1,1), stateArray0(1,2)];
rwind=0;%changes depending on boat params
currth=stateArray0(1,6);
aa=0;
tackpointx=0;
tackpointy=0;
waypoint=0;
...
while(ishandle(f))
    ...
    %ALEC EDIT: Insert nav code here, include all needed variables inside
    %and outside of loop, as well as take into account the "command"
    %variable (slides up and down instead of selecting angles to set sail
    %and tail) and adjust params to account for a given destination
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    wp1=[10, 10, 0, 0];
   %Get input from the keyboard
   if ishandle(f)
       out=nShort5(wp1,currpos, rwind, currth, waypoint, waypointsize, aa,tackpointx, tackpointy, prevNormr, timeAway, prevSail, prevTail);
       commands.up = out(10);
       commands.down = out(11);
       commands.left = out(12);
       commands.right = out(13);
       prevSail=p.sail.angle_relBody;
       prevTail=p.rudder.angle_relSail;
       aa=out(4);
        tackpointx=out(5);
        tackpointy=out(6);
        waypoint=out(7); 
        prevNormr=out(8);
        timeAway=out(9);
        currpos=[stateArray(stateAcount,1), stateArray(stateAcount,2)];
        currth=stateArray(stateAcount,6);
   end
   
   stateAcount=stateAcount+1;
