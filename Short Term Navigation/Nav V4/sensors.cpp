...
double tailMap(double sailAngle, double tailAngle){
  ...
  if (newTailAngle <= 0 ){
    newTailAngle=map(newTailAngle,-30,0,54,100); 
  }
  else if (newTailAngle > 0 ){
    newTailAngle=map(newTailAngle,0,30,100,160); 
  }
  return newTailAngle;
}
...
void sRSensor(void) {
  ...
  sensorData.windDir = 0; // constant wind for testing
  ...
  Serial.print("stored value for wind direction: "); Serial.println(sensorData.windDir);
  Serial1.print("stored value for wind direction: "); Serial1.println(sensorData.windDir);
}
